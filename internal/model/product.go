...
type Product struct {
	...
	Status    ProductStatus    `gorm:"column:status;check:status IN ('available', 'sold', 'removed');default:available;index:idx_cat_status;index:idx_status" json:"status"` // 状态
	Condition ProductCondition `gorm:"column:condition;check:status IN ('new', 'like_new', 'good', 'fair', 'poor');comment:新旧程度;not null;" json:"condition"`                 // 新旧程度
